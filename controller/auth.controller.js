...
export const postLogin = async (req, res) => {
  try {
    ...
    const isValid = await verifyPassword(password, existEmail.password);
    // console.log(isValid);
    if (!isValid) {
    ...
    const session = await createSession(existEmail._id, {
      ip: req.clientIp,
      userAgent: req.headers["user-agent"],
    });
...
export const editProfileNamePage = async (req, res) => {
  ...
  return res.render("auth/edit-profile-page", { name: user.username });
};
